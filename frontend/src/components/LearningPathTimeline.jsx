...
const getPhase = (week, totalWeeks) => {
  ...
  if (week <= quarter) return phaseConfig[0];
  if (week <= quarter * 2) return phaseConfig[1];
  if (week <= quarter * 3) return phaseConfig[2];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        {phaseConfig.map((p) => (
          <span key={p.label} className={lex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium   border }>
            ...
            <div key={week.week} className={ounded-2xl border p-5  }>
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className={lex h-8 w-8 items-center justify-center rounded-full  text-sm font-bold text-white}>
                    {week.week}
                  </span>
                  <div>
                    <p className={	ext-sm font-semibold }>{phase.label}</p>
